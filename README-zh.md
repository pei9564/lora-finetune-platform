...
一個由**一人獨立完成、歷時 30 天、以 AI 作為開發倍增器**所打造的全生命週期 MLOps 平台。
...
本專案深度實踐了人機協作的開發模式，全系統約 **80% 核心程式碼與配置由 ChatGPT-4o 與 Cursor 協作生成** — 這不是投機取巧，而是一種刻意選擇的工程方法論。

| 工具 | 角色定位 |
|------|---------|
| ChatGPT-4o | 架構規劃、設計取捨討論、技術文件撰寫 |
| Cursor | 編輯器內 Pair Programming、重構與快速迭代 |

**結果是：** 一個開發者，獨力設計、實作並交付了一個通常需要小型後端團隊才能完成的系統。

**AI 協作在實務上長什麼樣子？**

AI 很擅長快速生成看起來合理的程式碼，但生產系統的要求更高。即使生成的架構或函式能通過語法檢查，仍需要持續 debug、調整邏輯、測試整合性，才能建立出真正可運行的產品。隨著模組數量增加，早期程式碼往往缺乏通用性 — 若沒有測試覆蓋與清晰的模組邊界，重構的代價會急速攀升。

真正有效的策略是：**先寫測試，再疊功能。** AI 讓我能更快看見正確的方向；測試與實際的 debug，讓產品真正落了地。

> 架構決策、Prompt 指令集與人機調優筆記，均詳細記錄於[鐵人賽系列文章](https://ithelp.ithome.com.tw/users/20151660/ironman/8264)中。
